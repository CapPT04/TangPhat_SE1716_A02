...
const LecturerLayout: React.FC = () => {
  ...
  const menuItems: MenuItem[] = [
    { path: '/lecturer/my-news', label: 'My News', icon: <FileText className="w-5 h-5" /> },
    { path: '/lecturer/profile', label: 'My Profile', icon: <User className="w-5 h-5" /> },
  ];
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/10">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-gradient-to-br from-green-400 to-teal-400 rounded-full flex items-center justify-center text-white font-bold">
                  {user.accountName?.charAt(0).toUpperCase() || 'L'}
                </div>
        ...
        <header className="bg-slate-900/50 backdrop-blur-xl border-b border-white/10 sticky top-0 z-40">
          <div className="px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="px-4 py-2 bg-gradient-to-r from-green-500 to-teal-500 text-white rounded-lg text-sm font-medium shadow-lg">
                Lecturer Panel
              </div>
            </div>
          </div>
