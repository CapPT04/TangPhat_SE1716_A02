...
export const AdminLayout: React.FC = () => {
  ...
  const menuItems: MenuItem[] = [
    { path: '/admin/accounts', label: 'Account Management', icon: <Users className="w-5 h-5" /> },
    { path: '/admin/reports', label: 'Statistics Report', icon: <BarChart3 className="w-5 h-5" /> },
  ];
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/10">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-purple-400 rounded-full flex items-center justify-center text-white font-bold">
                  {user.accountName?.charAt(0).toUpperCase() || 'A'}
                </div>
                <div className="flex-1">
                  <p className="text-sm font-medium text-white">{user.accountName}</p>
                  <p className="text-xs text-slate-400">{user.accountEmail}</p>
                </div>
              </div>
              <button
