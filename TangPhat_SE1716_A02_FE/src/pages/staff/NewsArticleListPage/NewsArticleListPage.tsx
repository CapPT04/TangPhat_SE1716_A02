...
import { Plus, Search, Edit, Trash2, FileText, Filter, CheckCircle, XCircle, Calendar, User, Folder } from 'lucide-react';
...
const NewsArticleListPage = () => {
  ...
  const filteredNews = newsArticles.filter(news => {
    if (filterCategoryId > 0 && news.categoryId !== filterCategoryId) {
      return false;
    }
    return true;
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-blue-200">Create and manage news articles</p>
        </motion.div>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex gap-3 w-full lg:w-auto">
              <div className="relative flex-1 lg:flex-initial">
                <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-300" />
                <select
                  value={filterCategoryId}
                  onChange={(e) => setFilterCategoryId(parseInt(e.target.value))}
                  className="w-full lg:w-auto pl-12 pr-8 py-3 bg-white/5 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none"
                >
                  <option value={0} className="bg-gray-900">All Categories</option>
                  {categories.map((cat) => (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/10">
                    ...
                    <th className="px-6 py-4 text-left text-sm font-semibold text-blue-200">Author</th>
                ...
                <tbody>
                  {filteredNews.map((news, index) => (
                    ...
                    >
                      <td className="px-6 py-4 text-white font-mono">{news.newsArticleId}</td>
                      <td className="px-6 py-4">
                        <div 
                          className="text-white font-medium max-w-md cursor-pointer hover:text-blue-400 transition-colors"
                          onClick={() => navigate(`/staff/news/${news.newsArticleId}`)}
                        >
                          {news.newsTitle}
                        </div>
                        {news.headline && (
